...
public class AppHandler {
    ...
    private void preferences(HttpExchange exchange) throws IOException {
        ...
        try {
            dlLimitKb = cfg.getCoreLongParameter(PluginConfig.CORE_PARAM_LONG_MAX_DOWNLOAD_SPEED_KBYTES_PER_SEC);
        } catch (Exception ignored) {
        }
        try {
            ulLimitKb = cfg.getCoreLongParameter(PluginConfig.CORE_PARAM_LONG_MAX_UPLOAD_SPEED_KBYTES_PER_SEC);
        } catch (Exception ignored) {
        }
        ...
        try {
            maxConnTorrent = cfg.getCoreIntParameter(PluginConfig.CORE_PARAM_INT_MAX_CONNECTIONS_PER_TORRENT);
        } catch (Exception ignored) {
        }
