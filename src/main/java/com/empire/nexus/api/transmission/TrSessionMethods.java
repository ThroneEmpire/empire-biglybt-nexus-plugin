...
class TrSessionMethods {
    ...
    JsonObject get(JsonObject args) {
        ...
        long dlKB = 0, ulKB = 0;
        try {
            dlKB = pc.getCoreLongParameter(PluginConfig.CORE_PARAM_LONG_MAX_DOWNLOAD_SPEED_KBYTES_PER_SEC);
        } catch (Exception ignored) {
        }
        try {
            ulKB = pc.getCoreLongParameter(PluginConfig.CORE_PARAM_LONG_MAX_UPLOAD_SPEED_KBYTES_PER_SEC);
        } catch (Exception ignored) {
        }
        ...
        try {
            maxGlobal = pc.getCoreIntParameter(PluginConfig.CORE_PARAM_INT_MAX_CONNECTIONS_GLOBAL);
        } catch (Exception ignored) {
        }
        try {
            maxPerTorrent = pc.getCoreIntParameter(PluginConfig.CORE_PARAM_INT_MAX_CONNECTIONS_PER_TORRENT);
        } catch (Exception ignored) {
        }
        ...
        int port = 6881;
        try {
            port = pc.getCoreIntParameter(PluginConfig.CORE_PARAM_INT_INCOMING_TCP_PORT);
        } catch (Exception ignored) {
        }
    ...
    JsonObject stats() {
        ...
        try {
            allDl = pi.getDownloadManager().getStats().getOverallDataBytesReceived();
        } catch (Exception ignored) {
        }
        try {
            allUl = pi.getDownloadManager().getStats().getOverallDataBytesSent();
        } catch (Exception ignored) {
        }
