...
class TrTorrentMethods {
    ...
    private JsonObject buildTorrentFields(Download dl, int id, List<String> fields) {
        ...
        for (String field : fields) {
            try {
                addField(t, field, dl, torrent, stats, id);
            } catch (Exception ignored) {
            }
        }
        return t;
    ...
    private void addField(JsonObject t, String f, Download dl, Torrent torrent,
        ...
        switch (f) {
            ...
            case "have_valid" ->
                    t.addProperty("have_valid", (long) (torrent.getSize() * stats.getCompleted() / 1000.0));
            case "have_unchecked" -> t.addProperty("have_unchecked", 0L);
            case "left_until_done" -> {
                long left = torrent.getSize() - (long) (torrent.getSize() * stats.getCompleted() / 1000.0);
                t.addProperty("left_until_done", Math.max(0L, left));
            }
            case "desired_available" -> {
                long left = torrent.getSize() - (long) (torrent.getSize() * stats.getCompleted() / 1000.0);
                t.addProperty("desired_available", Math.max(0L, left));
            }
            ...
            case "file_count" -> {
                int fc = 0;
                try {
                    DiskManagerFileInfo[] fi = dl.getDiskManagerFileInfo();
                    fc = fi != null ? fi.length : 0;
                } catch (Exception ignored) {
                }
                t.addProperty("file_count", fc);
            }
            ...
            // ── ETA ───────────────────────────────────────────────────────────
            case "eta" -> {
                long e = stats.getETASecs();
                t.addProperty("eta", (e == Long.MAX_VALUE || e < 0) ? -1L : e);
            }
            ...
            case "added_date" -> {
                long ts = 0;
                try {
                    ts = stats.getTimeStarted() / 1000L;
                } catch (Exception ignored) {
                }
                t.addProperty("added_date", ts);
            }
            ...
            case "start_date" -> {
                ...
                t.addProperty("start_date", ts);
            }
            case "date_created" -> {
                long dc = 0;
                try {
                    dc = torrent.getCreationDate();
                } catch (Exception ignored) {
                }
                t.addProperty("date_created", dc);
            }
            ...
            case "download_dir" -> {
                String dir = "";
                try {
                    dir = stats.getDownloadDirectory();
                } catch (Exception ignored) {
                }
                t.addProperty("download_dir", dir != null ? dir : "");
            }
            case "comment" -> {
                String c = "";
                try {
                    c = torrent.getComment();
                } catch (Exception ignored) {
                }
                t.addProperty("comment", c != null ? c : "");
            }
            case "creator" -> {
                String c = "";
                try {
                    c = torrent.getCreatedBy();
                } catch (Exception ignored) {
                }
                t.addProperty("creator", c != null ? c : "");
            }
            case "magnet_link" ->
                    t.addProperty("magnet_link", "magnet:?xt=urn:btih:" + TorrentMapper.hashHex(torrent.getHash()) + "&dn=" + urlEncode(torrent.getName()));
            ...
            case "download_limit" -> {
                int lim = 0;
                try {
                    lim = dl.getDownloadRateLimitBytesPerSecond() / 1024;
                } catch (Exception ignored) {
                }
                t.addProperty("download_limit", lim);
            }
            case "download_limited" -> {
                boolean lim = false;
                try {
                    lim = dl.getDownloadRateLimitBytesPerSecond() > 0;
                } catch (Exception ignored) {
                }
                t.addProperty("download_limited", lim);
            }
            case "upload_limit" -> {
                int lim = 0;
                try {
                    lim = dl.getUploadRateLimitBytesPerSecond() / 1024;
                } catch (Exception ignored) {
                }
                t.addProperty("upload_limit", lim);
            }
            case "upload_limited" -> {
                boolean lim = false;
                try {
                    lim = dl.getUploadRateLimitBytesPerSecond() > 0;
                } catch (Exception ignored) {
                }
                t.addProperty("upload_limited", lim);
            }
            ...
            case "peers_connected" -> {
                ...
                try {
                    ...
                    if (pm != null) c = pm.getStats().getConnectedSeeds() + pm.getStats().getConnectedLeechers();
                } catch (Exception ignored) {
                }
                t.addProperty("peers_connected", c);
            }
            case "peers_sending_to_us" -> {
                int c = 0;
                try {
                    PeerManager pm = dl.getPeerManager();
                    if (pm != null) c = pm.getStats().getConnectedSeeds();
                } catch (Exception ignored) {
                }
                t.addProperty("peers_sending_to_us", c);
            }
            case "peers_getting_from_us" -> {
                int c = 0;
                try {
                    PeerManager pm = dl.getPeerManager();
                    if (pm != null) c = pm.getStats().getConnectedLeechers();
                } catch (Exception ignored) {
                }
                t.addProperty("peers_getting_from_us", c);
            }
    ...
    private List<Download> resolveIds(JsonObject args) {
        ...
        if (idsEl.isJsonPrimitive()) {
            ...
            if ("recently-active".equals(s) || "recently_active".equals(s)) {
                ...
                try {
                    Download dl = findById(Integer.parseInt(s));
            ...
            for (JsonElement e : idsEl.getAsJsonArray()) {
                try {
                    Download dl = findById(e.getAsInt());
                    if (dl != null) result.add(dl);
                } catch (Exception ignored) {
                }
            }
    ...
    private JsonArray buildTrackerStats(Download dl, Torrent torrent) {
        ...
        try {
            ...
            if (s != null && s.getResponseType() == DownloadScrapeResult.RT_SUCCESS) {
                seeds = s.getSeedCount();
                leechers = s.getNonSeedCount();
            }
        } catch (Exception ignored) {
        }
    ...
    private void applyFilePriorities(Download dl, JsonObject args) {
        ...
        try {
            files = dl.getDiskManagerFileInfo();
        } catch (Exception e) {
            return;
        }
        ...
        if (args.has("priority_high")) {
            ...
            for (int i = 0; i < files.length; i++)
                if (hi.contains(i)) try {
                    files[i].setNumericPriority(DiskManagerFileInfo.PRIORITY_HIGH);
                } catch (Exception ignored) {
                }
        }
        if (args.has("priority_normal")) {
            ...
            for (int i = 0; i < files.length; i++)
                if (normal.contains(i)) try {
                    files[i].setNumericPriority(DiskManagerFileInfo.PRIORITY_NORMAL);
                } catch (Exception ignored) {
                }
        }
        if (args.has("priority_low")) {
            ...
            for (int i = 0; i < files.length; i++)
                if (lo.contains(i)) try {
                    files[i].setNumericPriority(DiskManagerFileInfo.PRIORITY_LOW);
                } catch (Exception ignored) {
                }
        }
    }
